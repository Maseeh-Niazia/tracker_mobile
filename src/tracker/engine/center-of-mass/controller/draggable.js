...
var makeWindowDraggable = function( view ) {
  ...
  var start = function( evt ) {
    ...
    bbox = view.node.getBoundingClientRect();
    posO = events.user.position( evt );
        
    yMax = bboxO.height - bbox.height;
    xMax = bboxO.width - bbox.width;
  ...
  position.x = bboxO.width / 2 - bbox.width / 2;
  position.y = bboxO.height / 2 - bbox.height / 2;
  updateView( position );
};
